...
1. Classify the change.
   ...
   - Same-repo patch: produce `handoff/autofix` and let `autofix.atom.yml` materialize it.
...
5. Run the focused checks:
   ...
   - `actionlint -color`
   - `pnpm --filter @you-design/e2e test tests/packaged-smoke-workflow.test.ts`
...
Usually no. If the body is already final markdown, produce `handoff/comment` and use `comment.atom.yml`. If the body must be generated from artifacts, secrets, or report code, produce `handoff/report` and let `report.atom.yml` materialize and upsert it.
